...
//    private def addToFT(cell: FeatureCell,rest:Set[FeatureCell]):
  case class FeatureCell(name: String, up: Option[FeatureCell], loc: Loc):
    ...
    override def toString:String =
      s"$name[${up.map(_.name).getOrElse("-")}@${showLoc(loc)}]"

  case class Group(card:Card,lst:Set[Feature]):
    override def toString: String =
      if lst.isEmpty then "" else
        s"${show(card)}\n${ind(lst.toList.sortBy(_.loc.swap).mkString("\n"))}"
  ...
  case class FM(root:Feature, cs:List[Constraint]):
    ...
    override def toString: String =
      s"features\n${ind(root.toString)}${
        if cs.nonEmpty
//        then s"\nconstraints\n${cs.sortBy(kv=>kv._2.swap).map(x => ind(FeatExprParser.show(x._1))).mkString("\n")}"
        then s"\nconstraints\n${cs.map(x => ind(FeatExprParser.show(x._1))).mkString("\n")}"
        else ""
      }"
  ...
  def preprocess(fm:FMTable): FM =
    ...
    for (c,patt,loc) <- fm.card do // a cardinality restriction
      applyCard(res,c,loc,expandGrPatterns(patt,fm.root)) match
        case None => sys.error(s"Failed to add group '$patt' @ ${showLoc(loc)} (expanded: ${expandGrPatterns(patt,fm.root)}) - maybe they are not siblings or they are misspelled.")
    ...
    for (c,loc)<-fm.cs; f<-FeatExprParser.vars(c) do
      if !feats(f) then sys.error(s"Unknown feature $f @${showLoc(loc)}")
    FM(res,fm.cs)
  ...
  private def applyCard(f:Feature, c:Card, loc:Loc, g:Set[String]): Option[Feature] =
    f.gs.find(g2 => g.subsetOf(g2.lst.toSet.map(_.name))) match
      ...
      case None =>
        // println(s"no superset of $g found in ${f.gs.map(g2=>g2.lst.toSet.map(_.name)).mkString(" / ")}")
        var done = false
  ...
  private def expandGrPattern(p:String, f:FeatureTable):Set[String] =
    p.split('.').map(_.trim) match
      ...
      case Array(top,"*") => //findChildren(top,f)
        ...
        yield c.name
      case x =>
        sys.error(s"Unexpected pattern $p split over `.' as ${x.map(y=>s"'$y'").mkString("-")}")
 ...
 /////
  ...
  def validate(prods:Cf.Products,fm:FM): (List[String],Cf.Products) = // list of errors // todo: also updated prod!!
